...
#Numeric: Almacena números reales, incluyendo decimales. Ejemplo: 3.14.
 númerico=3.4
 #Interger: Almacena números enteros. Ejemplo: 5.
 integer=5
...
#date: fecha
 date=as.Date"1964/07/07"
 date=as.Date("2003/07/23")
...
median(x)
 #mediana aritmeticas
median(x)
 #desviacion stadart estadisticas
sd(x)
 #Varianza estadisticas
var(x)
 #logaritmo aritmeticas
log(x)
 #logartmo base 10 aritmeticas
log10(x)
 #revertir orden orden
rev(x)
 #valores unicos orden
unique(x)
 #ordenar orden
sort(x)
 #orden reverso orden
...
plot(altura, peso) #función base para generar graficos
imc <- (peso/altura)^2
plot(peso,imc)
...
data[c(1, 3, 2)] #se pone un corchete del marco de datos y seleccionamos el número
...
Sepalos_largo > 6
# se necesitan corchete para seleccionar datos
Sepalos_largo[Sepalos_largo > 6]
Petalos_largo[Petalos_largo < 5]
#para anidar condiciones podemos usar los operadores logicos and y or
Petalos_largo[Sepalos_largo > 6 & Petalos_largo > 6]

# Generación de datos
1:10 #genera datos del 1 al 10
...
#Relevamiento de rindes, (50 parcelas, media de 2,8 t/ha, sd=0,8)
 ...
 #n = observaciones, mean = media, sd = desviacion standart
